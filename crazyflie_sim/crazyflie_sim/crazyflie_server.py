...
class CrazyflieServer(Node):
    ...
    def __init__(self):
        ...
        print(f"DEBUG: robot_types = {self._ros_parameters['robot_types']}")

        for cf_name in robot_data:
            print(f"DEBUG: Processing robot {cf_name}")
            if robot_data[cf_name]["enabled"]:
                type_cf = robot_data[cf_name]["type"]
                print(f"DEBUG: Robot {cf_name} type = {type_cf}")
                connection = self._ros_parameters["robot_types"][type_cf].get("connection", "crazyflie") 
                print(f"DEBUG: Robot {cf_name} connection = {connection}")
                if connection == "crazyflie":
                    names.append(cf_name)
                    print(f"DEBUG: Added {cf_name} to names list")
                    pos = robot_data[cf_name]['initial_position']
                    initial_states.append(State(pos))
                    print(f"DEBUG: Added initial state for {cf_name} at position {pos}")

        print(f"DEBUG: Final names list = {names}")
        print(f"DEBUG: Final initial_states count = {len(initial_states)}")
        '''
        ...
        for name, _ in self.cfs.items():
            ...
            self.create_subscription(VelocityWorld, f'{name}/cmd_velocity_world', partial(self._cmd_velocity_world_changed, name=name), 10)
